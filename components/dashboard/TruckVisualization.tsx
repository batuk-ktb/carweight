...
const TruckVisualization: React.FC<TruckVisualizationProps> = ({
  ...
  // 4 visual sections
  // top = odd axles (AX-01, AX-03, AX-05, AX-07)
  // bottom = even axles (AX-02, AX-04, AX-06, AX-08)
  const sections = [
    { num: 1, top: { id: containerId1, axle: "Баруун чингэлэг: 1" }, bottom: { id: containerId2, axle: "Зүүн чингэлэг: 1" } },
    { num: 2, top: { id: containerId3, axle: "Баруун чингэлэг: 2" }, bottom: { id: containerId4, axle: "Зүүн чингэлэг: 2" } },
    { num: 3, top: { id: containerId5, axle: "Баруун чингэлэг: 3" }, bottom: { id: containerId6, axle: "Зүүн чингэлэг: 3" } },
    { num: 4, top: { id: containerId7, axle: "Баруун чингэлэг: 4" }, bottom: { id: containerId8, axle: "Зүүн чингэлэг: 4" } },
  ];

  const sectionW = 110;
  const totalW = sectionW * 4;

  const TopBadge = ({ axle, id }: { axle: string; id: string }) => (
    <div
      style={{
        width: "100%",
  ...
  const BottomBadge = ({ axle, id }: { axle: string; id: string }) => (
    <div
      style={{
        width: "100%",
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, position: "relative" }}>
        <div>
          <div style={{ fontSize: 9, letterSpacing: "0.3em", color: "rgba(255,255,255,0.5)", textTransform: "uppercase", marginBottom: 2 }}>
            WIM — Container Tracking
          </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 20, padding: "4px 12px" }}>
          <span style={{
            ...
            boxShadow: pulse ? "0 0 10px #fff" : "0 0 2px rgba(255,255,255,0.5)",
      ...
      {/* TOP ROW — AX-01, AX-03, AX-05, AX-07 */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, marginBottom: 6 }}>
        {sections.map((s, i) => (
          <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
            <TopBadge axle={s.top.axle} id={s.top.id} />
      ...
      <svg viewBox={`0 0 ${totalW} 50`} style={{ width: "100%", display: "block", filter: "drop-shadow(0 4px 20px rgba(255,255,255,0.06))" }}>
        ...
        {/* Sections with big numbers */}
        {sections.map((s, i) => (
          <g key={i}>
            <rect
              x={i * sectionW + 3} y="5" width={sectionW - 6} height="37" rx="2"
